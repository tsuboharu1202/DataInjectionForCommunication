...
% 後で
dPi_dD = proposed.gradient.helper.dPi_dD(n,m,T,Gamma);
term1 = 2*gamma*kron(L',speye(T))*dPi_dD;
term2 = -proposed.gradient.helper.dVec_Lambda_TdFdL_dD(n,m,T,Lambda);

C2nmT = core.helper.commutation(2*n+m,T);
Ex = [sparse(n,n), speye(n),sparse(n,m)];
KTx = core.helper.commutation(T,n);
...
M   = Lambda3 - Lambda3';
KTx = core.helper.commutation(T,n);        % vec(dX') = KTx * vec(dX)
