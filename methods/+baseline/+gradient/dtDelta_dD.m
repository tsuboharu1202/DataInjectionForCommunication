...
G1_sol = baseline.gradient.G_grad.G1_merge(n,m,B,T);
G2_sol = baseline.gradient.G_grad.G2_merge(n,m,B,T);
G3_sol = baseline.gradient.G_grad.G3_merge(n,m,T,Lambda1,F2,B,G,Phi);
G4_sol = baseline.gradient.G_grad.G4_merge(n,m,T);
G5_sol = baseline.gradient.G_grad.G5_merge(n,m,T,B);
G6_sol = baseline.gradient.G_grad.G6_merge(n,m,T,Lambda1,alpha,F1,F2,B,G,Phi);
G7_sol = baseline.gradient.G_grad.G7_merge(n,m,T,Lambda3,F3);
G8_sol = baseline.gradient.G_grad.G8_merge(n,m,T,alpha,Lambda_alpha);
G9_sol = baseline.gradient.G_grad.G9_merge(n,m,T,beta,Lambda_beta);
G10_sol = baseline.gradient.G_grad.G10_merge(n,m,T,tDelta,Lambda_tDelta);
G11_sol = baseline.gradient.G_grad.G11_merge(n,m,T);
G12_sol = baseline.gradient.G_grad.G12_merge(n,m,T);
G13_sol = baseline.gradient.G_grad.G13_merge(n,m,T);
G14_sol = baseline.gradient.G_grad.G14_merge(n,m,T,Lambda_Y,Y);
G15_sol = baseline.gradient.G_grad.G15_merge(n,m,T,Lambda_Y);
