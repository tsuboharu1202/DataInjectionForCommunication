% function [M25, M26, M27] = build_lmi_blocks_perturbation(Y, L, alpha, beta, tTau, tDelta, H, E1, E2, Phi,Z,Xm,Um);
% % sizes
% n  = size(Y,1);
% m  = size(L,1);
% n2 = size(E2,1);
...
% Zmn2 = zeros(m,n2);  Zn2m = zeros(n2,m);
...
% D = [eye(2*n+m);
%     Z', Xm', Um'];
% Phi_new = [1e-6*(2*n+m)*cfg.Const.SAMPLE_COUNT*eye(2*n+m), zeros(2*n+m, cfg.Const.SAMPLE_COUNT);
%     zeros(cfg.Const.SAMPLE_COUNT, 2*n+m), -eye(cfg.Const.SAMPLE_COUNT)];
% N = [D' * Phi_new * D ,zeros(2*n+m,n2+n+1+m)
%     zeros(n2+n+1+m,2*n+m), zeros(n2+n+1+m,n2+n+1+m)];
