...
& (X L)_\text{sym} \geq \epsilon I_n \label{eq:const_XL_reg} \\
...
\item $\epsilon > 0$: 数値的安定性のための小さな正定数（通常$10^{-8}$）
\end{itemize}
...
&\quad - \text{tr}(((X L)_\text{sym} - \epsilon I_n) \Lambda_P^T) \label{eq:Lagrangian_XL_reg}
\end{align}

ここで、ラグランジュ乗数は：
\begin{itemize}
\item $\Lambda_1 \in \mathbb{R}^{(2n+2m) \times (2n+2m)}$: $F_1 \geq 0$に対する双対変数（対称）
\item $\Lambda_P \in \mathbb{R}^{n \times n}$: $(X L)_\text{sym} \geq \epsilon I_n$に対する双対変数（対称）
\end{itemize}

\textbf{注意：} 実装では$\text{tr}(X \Lambda^T)$の形式を使用する。これは$\text{tr}(\Lambda X)$と等価であるが、微分計算において順序が重要となる。

\textbf{注意：} 等号制約$X L = (X L)^T$は、制約条件として直接扱われ、双対変数は使用しない。
...
G_5 &: \quad ((X L)_\text{sym} - \epsilon I_n) \Lambda_P^T = 0_{n \times n} \label{eq:G5_reg}
\end{align}
...
\begin{align}
\text{vec}\left(\frac{\partial F_1}{\partial \delta}\right) &= \begin{bmatrix}
0 \\
\vdots \\
0 \\
\text{vec}(B) \\
...
\vdots \\
0
\end{bmatrix} \in \mathbb{R}^{(2n+2m)^2} \label{eq:vec_dF1_ddelta}
\end{align}
...
\begin{align}
G_2 &= 2\gamma \frac{\partial}{\partial L} \| \Pi L \|_F^2 - \text{tr}\left(\frac{\partial F_1}{\partial L} \Lambda_1^T\right) - \text{tr}\left(\frac{\partial ((X L)_\text{sym})}{\partial L} \Lambda_P^T\right) \label{eq:G2_detail}
\end{align}
...
$F_1$の$L$に関する偏微分は、各ブロックを$L$で微分することで得られる。ベクトル化すると：

\begin{align}
\frac{\partial \text{vec}(F_1)}{\partial \text{vec}(L)} = \frac{\partial F_1}{\partial L} \in \mathbb{R}^{(2n+2m)^2 \times Tn} \label{eq:dF1_dL_vec}
\end{align}

$(X L)_\text{sym}$の$L$に関する偏微分は：

\begin{align}
\frac{\partial ((X L)_\text{sym})}{\partial L} &= \frac{1}{2}\left(\frac{\partial (X L)}{\partial L} + \frac{\partial ((X L)^T)}{\partial L}\right) \label{eq:dXLsym_dL}
\end{align}

したがって：

\begin{align}
G_2 &= 2\gamma \text{vec}(\Pi L) - \left(\frac{\partial F_1}{\partial L}\right)^T \text{vec}(\Lambda_1) - \left(\frac{\partial ((X L)_\text{sym})}{\partial L}\right)^T \text{vec}(\Lambda_P) = 0 \label{eq:G2_final}
\end{align}
...
\textbf{重要な点：} $G_3$を$\delta$で微分する際、$\frac{\partial F_1}{\partial \delta}$の項が現れる。これにより、$G_3$から$\frac{d\delta}{dD}$に関する項が得られる：

\begin{align}
\frac{\partial G_3}{\partial \delta} &= (I_{(2n+2m)} \otimes \Lambda_1) \text{vec}\left(\frac{\partial F_1}{\partial \delta}\right) \label{eq:G3_ddelta}
\end{align}

したがって：

\begin{align}
G_3: \quad (I_{(2n+2m)} \otimes \Lambda_1) \frac{\partial \text{vec}(F_1)}{\partial D} + (\Lambda_1 \otimes I_{(2n+2m)}) \frac{\partial \text{vec}(\Lambda_1^T)}{\partial D} + (I_{(2n+2m)} \otimes \Lambda_1) \text{vec}\left(\frac{\partial F_1}{\partial \delta}\right) \frac{d\delta}{dD} = 0 \label{eq:G3_final}
\end{align}
...
\subsection{$G_5$: 相補性条件 $((X L)_\text{sym} - \epsilon I_n) \Lambda_P^T = 0$}

\begin{align}
G_5: \quad \frac{\partial}{\partial D} (((X L)_\text{sym} - \epsilon I_n) \Lambda_P^T) = \frac{\partial ((X L)_\text{sym})}{\partial D} \Lambda_P^T + ((X L)_\text{sym} - \epsilon I_n) \frac{\partial \Lambda_P^T}{\partial D} = 0 \label{eq:G5_final}
\end{align}

ベクトル化すると：

\begin{align}
G_5: \quad (I_n \otimes \Lambda_P) \frac{\partial \text{vec}((X L)_\text{sym})}{\partial D} + (\Lambda_P \otimes I_n) \frac{\partial \text{vec}(\Lambda_P^T)}{\partial D} = 0 \label{eq:G5_vec}
\end{align}
...
\begin{align}
G_7: \quad (I - K_{n,n}) \frac{\partial \text{vec}(X L)}{\partial D} = 0 \label{eq:G7_vec}
\end{align}
...
ここで、$\bm{x} = [\delta, \text{vec}(L), \text{vec}(\Lambda_1), \text{vec}(\Lambda_P)]^T$ である。

変数の次元：
\begin{itemize}
\item $\delta$: $1$
\item $\text{vec}(L)$: $Tn$
\item $\text{vec}(\Lambda_1)$: $(2n+2m)^2$
\item $\text{vec}(\Lambda_P)$: $n^2$
\item \textbf{合計}: $1 + Tn + (2n+2m)^2 + n^2$
\end{itemize}
...
\item $G_6$: $n^2$
\item $G_7$: $n^2$
\item \textbf{合計}: $1 + Tn + 2(2n+2m)^2 + 3n^2$
\end{itemize}
...
\subsection{$G_3$における$\delta$の項}

$G_3$を$\delta$で微分する際、$\frac{\partial F_1}{\partial \delta}$の項が現れる。これにより、$G_3$から$\frac{d\delta}{dD}$に関する項が得られる。実装では、$G_3$の$\text{vec}(\delta)$に関する係数行列に、$(I_{(2n+2m)} \otimes \Lambda_1) \text{vec}\left(\frac{\partial F_1}{\partial \delta}\right)$を追加する必要がある。
...
\subsection{データ注入攻撃手法}

計算された勾配$\frac{d\delta}{dD}$を用いて、データ注入攻撃を実行する。実装では以下の2つの手法を提供している：

\subsubsection{DGSM (Direct Gradient Sign Method)}

DGSMは1ステップの攻撃手法であり、勾配の符号方向に一定の強度$\varepsilon$でノイズを加える：

\begin{align}
D_{\text{adv}} = D + \varepsilon \cdot \text{sign}\left(\frac{d\delta}{dD}\right) \cdot \text{dir} \label{eq:dgsm}
\end{align}

ここで、$\text{dir} \in \{+1, -1\}$は攻撃方向を表す：
\begin{itemize}
\item $\text{dir} = +1$: $\delta$を大きくする方向（positive方向）
\item $\text{dir} = -1$: $\delta$を小さくする方向（negative方向、デフォルト）
\end{itemize}

\subsubsection{IDGSM (Iterative Direct Gradient Sign Method)}

IDGSMは反復的な攻撃手法であり、各反復で勾配を計算し、ノイズを累積する：

\begin{align}
d^{(k+1)} &= d^{(k)} + \alpha \cdot \tilde{g}^{(k)} \cdot \text{dir} \label{eq:idgsm_update} \\
D_{\text{adv}}^{(k+1)} &= D + \text{Proj}_{\varepsilon}(d^{(k+1)}) \label{eq:idgsm_proj}
\end{align}

ここで：
\begin{itemize}
\item $d^{(k)}$: $k$回目の反復での累積ノイズ
\item $\tilde{g}^{(k)}$: 正規化された勾配（後述）
\item $\alpha > 0$: ステップサイズ（通常$5 \times 10^{-3}$程度）
\item $\text{Proj}_{\varepsilon}(\cdot)$: 要素単位のL∞ノルム制約を満たす投影演算子
\end{itemize}

\subsubsection{勾配の正規化}

IDGSMでは、各勾配$g^{(k)} = [g_X^{(k)}, g_Z^{(k)}, g_U^{(k)}]$を要素単位の最大値で正規化する：

\begin{align}
\tilde{g}_X^{(k)} &= \frac{g_X^{(k)}}{\max_{i,j} |g_X^{(k)}(i,j)|} \label{eq:norm_X} \\
\tilde{g}_Z^{(k)} &= \frac{g_Z^{(k)}}{\max_{i,j} |g_Z^{(k)}(i,j)|} \label{eq:norm_Z} \\
\tilde{g}_U^{(k)} &= \frac{g_U^{(k)}}{\max_{i,j} |g_U^{(k)}(i,j)|} \label{eq:norm_U}
\end{align}

この正規化により：
\begin{itemize}
\item 各勾配の最大要素が1になり、ステップサイズ$\alpha$の効果が明確になる
\item 投影演算子$\text{Proj}_{\varepsilon}$の要素単位制約（L∞ノルム）と整合性がある
\item 勾配のスケールが小さい場合でも、$\alpha$の効果が適切に反映される
\end{itemize}

\subsubsection{投影演算子}

投影演算子$\text{Proj}_{\varepsilon}$は、要素単位でL∞ノルム制約を適用する：

\begin{align}
\text{Proj}_{\varepsilon}(d)(i,j) = \begin{cases}
\varepsilon \cdot \text{sign}(d(i,j)) & \text{if } |d(i,j)| > \varepsilon \\
d(i,j) & \text{otherwise}
\end{cases} \label{eq:projector}
\end{align}

この投影により、各要素が$[-\varepsilon, \varepsilon]$の範囲内に制限される。
...
\item \textbf{等号制約}: 正則化版は$X L = (X L)^T$を等号制約として直接扱い、双対変数を使用しない
\end{enumerate}
...
特に重要な点は、$G_3$における$\delta$の項を正しく扱うことで、システムのrankが満たされ、解の一意性が保証されることである。

計算された勾配を用いて、DGSM（1ステップ攻撃）やIDGSM（反復攻撃）によりデータ注入攻撃を実行できる。IDGSMでは、要素単位の最大値による正規化により、勾配のスケールが小さい場合でも適切に動作し、投影演算子による要素単位のL∞ノルム制約と整合性が保たれる。
