...
%
%   作成ファイル一覧:
%       - implicit_regularization.helper.F1_of_L.m
%       - implicit_regularization.helper.test_dF_dL_FD.m (このファイル)
%
%   呼び出し例:
...
%
%       % 一行で呼び出し:
%       implicit_regularization.helper.test_dF_dL_FD(n, m, T, B, X, Z, U, L_val, delta_val);
%
%   または、名前空間を使わずに:
%       test_dF_dL_FD(n, m, T, B, X, Z, U, L_val, delta_val);
...
% F1_of_Lのサイズ確認
F1_test = implicit_regularization.helper.F1_of_L(n, m, T, B, X, Z, U, L_val, delta_val);
...
% ベースライン: F1(L_val)
F1_base = implicit_regularization.helper.F1_of_L(n, m, T, B, X, Z, U, L_val, delta_val);
vec_F1_base = F1_base(:);
...
for trial = 1:nTrials
    ...
    for eps_idx = 1:length(eps_list)
        ...
        F1_perturbed = implicit_regularization.helper.F1_of_L(n, m, T, B, X, Z, U, L_perturbed, delta_val);
