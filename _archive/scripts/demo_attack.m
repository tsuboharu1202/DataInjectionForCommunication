...
% gamma = 1e3;
% [sol_init, ~, ~, ~, ~] = regularization_sdp.solve_sdp_non_robust(data, [], gamma);
...
% [X_idgsm_neg, Z_idgsm_neg, U_idgsm_neg, history_idgsm_neg] = attack.idgsm_delta(data, true, [], [], eps_att, 'negative', true, opts_normalize);
...
% [X_idgsm_neg_no_norm, Z_idgsm_neg_no_norm, U_idgsm_neg_no_norm, history_idgsm_neg_no_norm] = attack.idgsm_delta(data, true, [], [], eps_att, 'negative', true, opts_no_normalize);
...
% [sol_idgsm_neg_no_norm, ~, ~, ~, ~] = regularization_sdp.solve_sdp(data_idgsm_neg_no_norm, gamma);
...
% % 勾配ノルムを記録するためにsave_historyとsave_grad_normsを有効化
% [X_idgsm_pos, Z_idgsm_pos, U_idgsm_pos, history_idgsm_pos] = attack.idgsm_delta(data, true, [], [], eps_att, 'positive', true, opts);
